...
可以知道七天睡眠時間的平均值$\overline{x} = \dfrac{8+8+7+7+7+5+7}{7} = 7$小時
...
若我們要取得$50th$百分位數的位置，可以得到$L_{50} = (\dfrac{50}{100})(100+1) = 50.5$
...
我們想要知道有多少的學生的分數介於$60$到$80$之間，又有多少的學生的分數介於$58$到$82$之間。
...
$$
s_{xy} = \dfrac{\sum (x_i - \overline{x})(y_i-\overline{y})}{n-1}
$$
而對於母體大小$N$的共變異數定義如下：
$$
\sigma_{xy} = \dfrac{\sum (x_i - \mu_x)(y_i - \mu_y)}{N}
$$


#### Introduce - 共變異數的解釋

我們可以用$x=\overline{x}$軸與$y=\overline{y}$軸分割成四個平面，接著我們考慮

- 如果點落在第一象限與第三象限，則$(x_i - \overline{x})(y_i - \overline{y}) > 0$

  可以推導出$x$與$y$有正的線性相關，也就是當$x$值增加，$y$值也增加。

- 如果點落在第二象限與第四象限，則$(x_i - \overline{x})(y_i - \overline{y}) < 0$

  可以推導出$x$與$y$有負的線性相關，也就是當$x$值增加，$y$值則減少。

- 如果點落的非常的散，則$s_{xy}$的值會趨近於$0$，表示$x$與$y$沒有線性關係
...
$$
其中$\sigma_{xy}$是樣本的共變異數，且$\sigma_x$是$x$的樣本標準差，$\sigma_y$是$y$的樣本標準差。
...
如果是毫無相關的話，則$\sigma_{xy} \approx 0$，因此相關係數也會趨近於$0$，因此$r_{xy} \approx 0$時代表線性關係微弱



## Chapter 4 機率導論

### Section 4.1 隨機實驗、計數法則及機率指派

#### Introduce - 隨機實驗

若一個實驗符合以下特性，我們稱這個實驗為隨機實驗：

1. 實驗結果有清楚定義，大多情況下，甚至可以在實驗之前就列出所有可能結果
2. 實驗的任何重複進行或試行，都會出現一個且僅一個實驗結果
3. 出現哪種實驗結果全憑機會而定

也可以轉成以下的敘述

> 能產生定義清楚的結果的程序，實驗的任何重複或試行所出現的結果完全由機率決定。
...
樣本空間為隨機實驗的所有可能實驗結果的集合。

例如一個硬幣的實驗結果可能有正面與反面，所以我們可以用這樣表示樣本空間。
$$
S=\{正面, 反面\}
$$
一個骰子所骰出來的樣本空間可以這樣表示。
...
若有一個實驗可以分成$k$個步驟，而第一個步驟有$n_1$個可能結果，第二個步驟有$n_2$個，以此類推。

則這個實驗共有$n_1n_2n_3...n_k$個實驗結果。



例如我們想要知道丟兩次硬幣的樣本空間，可以發現有四種結果，因為$2\cdot 2 = 4$。

我們有$T$代表反面，$H$代表正面，則可以得到以下的樣本空間。
$$
S=\{(H,T), (T, H), (H, H), (T, T)\}
$$
如果丟六次，則有$64$種結果，因為$2\cdot2\cdot2\cdot2\cdot2\cdot2 = 64$



#### Introduce - 樹狀圖

我們可以把每一個可能的情況畫成樹狀圖，得到以下的圖

<img src="https://i.imgur.com/L1arHLk.png" alt="image-20211024170325078" style="zoom:67%;" />

就能夠從圖中得知，還沒擲之前這四個實驗結果都可能發生，當第一個出現的是正面，則剩下兩個實驗結果會發生。



#### Introduce - 組合

我們可以用組合來求得在$N$個物品中取出$n$個物品可能的組合數量，共有：
$$
C^N_n = \binom{N}{n} = \dfrac{N!}{n!(N-n)!}
$$
其中
$$
N! = N(N-1)(N-2)...(2)(1) \\
...
我們可以用排序來求得在$N$個物品中取出$n$個物品，同時考慮取出的順序，共有：
$$
P^N_n = n! \binom{N}{n} = \dfrac{N!}{(N-n)!}
$$


#### Introduce - 機率指派

我們用三種最常用的方法來指派各實驗結果的機率值
...
1. 指派到任一實驗結果的機率值必須介於0到1之間，假設$E_i$表示第$i$個實驗結果，而$P(E_i)$表示該事件發生的機率，則對於所有$i$而言，$0 \le P(E_i) \le 1$
2. 所有實驗結果出現的機率總和必須等於$1$，也就是若樣本空間含有$n$個實驗結果，則$P(E_1) + P(E_2) + ... + P(E_n) = 1$



#### Introduce - 古典法

古典法適用於，若有$n$個實驗結果，各實驗結果出現的機率均為$1/n$。

例如一個硬幣投擲，正反面出現的機率均為$1/2$。

或者一個骰子投擲，每一面出現的機率均為$1/6$。
...
在一個重複很多次的實驗中，我們以某個實驗結果佔實驗次數總和的比例當成機率，我們稱為相對次數法。

例如若投擲某個硬幣500次，且出現正面的機率是250次，則我們利用相對次數法可以得知機率為$250/500=0.5=50\%$



#### Introduce - 主觀法

主觀法對實驗結果進行機率指派時使用的方式為，單純表示個人的信心程度，因此不同的人對相同的事可能有不同的機率指派。



例如：假設我們指派兩種結果，分別代表Uriah會遲到$(E_1)$，以及Uriah不會遲到$(E_2)$。

今天是北科程式設計社的開會現場，而Uriah自己本身覺得遲到機率$E_1=0.8$，不會遲到的機率$E_2=0.2$

但社團幹部們認為Uriah的遲到機率為$E_1=0.99$，且不會遲到的機率為$E_2=0.01$

這樣就代表Uriah對於估計自己會遲到的機率比較偏高。
